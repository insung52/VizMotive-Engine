...
namespace vz::graphics
{
	...
	public:
		...
		virtual bool CreateSwapChain(const SwapChainDesc* desc, vz::platform::window_type window, SwapChain* swapchain) const = 0;
		...
		// Upload data to a specific region of an existing buffer (for suballocation offset-based uploads)
		virtual void UploadToBufferRegion(GPUBuffer* buffer, uint64_t offset, const void* data, uint64_t size) const = 0;

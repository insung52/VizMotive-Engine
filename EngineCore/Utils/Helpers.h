...
namespace vz::helper 
{
	...
	// Cross-platform deterministic UTF-8 <-> wchar_t conversion
	// Note: these are implemented independently of platform APIs for consistent behavior
	...
	// dest_size_in_characters : number of characters in the pre-allocated string memory (REQUIRED)
	// returns result string length
	inline int StringConvert(const char* from, wchar_t* to, int dest_size_in_characters)
	{
	...
	// dest_size_in_characters : number of characters in the pre-allocated string memory (REQUIRED)
	// returns result string length
	inline int StringConvert(const wchar_t* from, char* to, int dest_size_in_characters)
	{
		if (!from || !to || dest_size_in_characters <= 0)
			return 0;

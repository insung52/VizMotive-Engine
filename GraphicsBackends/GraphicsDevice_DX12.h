...
namespace vz::graphics
{
	...
	protected:
		...
		struct CommandQueue
		{
			...
			std::vector<ID3D12CommandList*> submit_cmds;
		...
		std::vector<Semaphore> semaphore_pool;
		...
		struct CommandList_DX12
		{
			...
			std::vector<std::pair<PipelineHash, Microsoft::WRL::ComPtr<ID3D12PipelineState>>> pipelines_worker;
			...
			std::vector<D3D12_RESOURCE_BARRIER> renderpass_barriers_begin_after_discards;
			...
			std::vector<D3D12_RENDER_PASS_ENDING_ACCESS_RESOLVE_SUBRESOURCE_PARAMETERS> resolve_subresources[D3D12_SIMULTANEOUS_RENDER_TARGET_COUNT] = {};
			std::vector<D3D12_RENDER_PASS_ENDING_ACCESS_RESOLVE_SUBRESOURCE_PARAMETERS> resolve_subresources_dsv = {};
			std::vector<D3D12_RESOURCE_BARRIER> resolve_src_barriers;
		...
		std::unordered_map<PipelineHash, Microsoft::WRL::ComPtr<ID3D12PipelineState>> pipelines_global;

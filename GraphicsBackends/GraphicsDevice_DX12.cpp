...
#include "D3D12MemAlloc.cpp" // include this here because we use D3D12MA_D3D12_HEADERS_ALREADY_INCLUDED
#include "ThirdParty/h264.h"
#include "ThirdParty/dxva.h"
...
namespace dx12_internal
{
	...
	constexpr UINT _ImageAspectToPlane(ImageAspect value)
	{
		...
		{
		default:
		case vz::graphics::ImageAspect::COLOR:
			return 0;
		case vz::graphics::ImageAspect::DEPTH:
			return 0;
		case vz::graphics::ImageAspect::STENCIL:
			return 1;
		case vz::graphics::ImageAspect::LUMINANCE:
			return 0;
		case vz::graphics::ImageAspect::CHROMINANCE:
			return 1;
		}
	}
	constexpr D3D12_SHADER_COMPONENT_MAPPING _ConvertComponentSwizzle(ComponentSwizzle value)
	{
		...
		{
		default:
		case vz::graphics::ComponentSwizzle::R:
			return D3D12_SHADER_COMPONENT_MAPPING_FROM_MEMORY_COMPONENT_0;
		case vz::graphics::ComponentSwizzle::G:
			return D3D12_SHADER_COMPONENT_MAPPING_FROM_MEMORY_COMPONENT_1;
		case vz::graphics::ComponentSwizzle::B:
			return D3D12_SHADER_COMPONENT_MAPPING_FROM_MEMORY_COMPONENT_2;
		case vz::graphics::ComponentSwizzle::A:
			return D3D12_SHADER_COMPONENT_MAPPING_FROM_MEMORY_COMPONENT_3;
		case vz::graphics::ComponentSwizzle::ZERO:
			return D3D12_SHADER_COMPONENT_MAPPING_FORCE_VALUE_0;
		case vz::graphics::ComponentSwizzle::ONE:
	...
	struct RootSignatureOptimizer
	{
		...
		std::vector<RootParameterStatistics> root_stats;
	...
	struct PipelineState_DX12
	{
		...
		const D3D12_VERSIONED_ROOT_SIGNATURE_DESC* rootsig_desc = nullptr;
		vz::allocator::shared_ptr<void> rootsig_desc_lifetime_extender;
...
#endif // PLATFORM_XBOX
		std::vector<vz::allocator::shared_ptr<Resource_DX12>> textures; // shared_ptr is used because they can be given out by GetBackBuffer()
...
std::mutex queue_locker;
	...
	GraphicsDevice_DX12::GraphicsDevice_DX12(ValidationMode validationMode_, GPUPreference preference)
	{
		...
		hr = vz::graphics::xbox::CreateDevice(device, dxgiAdapter, validationMode);
...
#ifdef PLATFORM_XBOX
		adapterName = vz::graphics::xbox::GetAdapterName();
		...
		if (features.HighestShaderModel() < D3D_SHADER_MODEL_6_0)
		{
			...
			switch (adapterType)
			{
			case vz::graphics::AdapterType::IntegratedGpu:
				error += "Integrated GPU";
				break;
			case vz::graphics::AdapterType::DiscreteGpu:
				error += "Discrete GPU";
				break;
			case vz::graphics::AdapterType::VirtualGpu:
				error += "Virtual GPU";
				break;
			case vz::graphics::AdapterType::Cpu:
				error += "CPU";
				break;
			default:
				error += "Unknown";
...
#ifdef PLATFORM_XBOX
			vz::graphics::xbox::FillDispatchMeshIndirectArgumentDesc(dispatchMeshArgs[0], cmd_desc);
#else
	...
	{
		auto internal_state = swapchain->IsValid() ? vz::allocator::shared_ptr<SwapChain_DX12>(swapchain->internal_state) : vz::allocator::make_shared<SwapChain_DX12>();
		...
		{
			internal_state->textures[i] = vz::allocator::make_shared<Resource_DX12>();
		...
		for (uint32_t i = 0; i < desc->buffer_count; ++i)
		{
			...
			hr = internal_state->swapChain->GetBuffer(i, PPV_ARGS(internal_state->textures[i]->resource));
			assert(SUCCEEDED(hr));
			internal_state->textures[i]->rtv.init(this, rtv_desc, internal_state->textures[i]->resource.Get());
			internal_state->textures[i]->srv.init(this, srv_desc, internal_state->textures[i]->resource.Get());
		}
...
#ifdef PLATFORM_XBOX
		vz::graphics::xbox::ApplyBufferCreationFlags(*desc, resourceDesc.Flags, allocationDesc.ExtraHeapFlags);
#endif // PLATFORM_XBOX
...
#ifdef PLATFORM_XBOX
		vz::graphics::xbox::ApplyTextureCreationFlags(texture->desc, resourcedesc.Flags, allocationDesc.ExtraHeapFlags);
#endif // PLATFORM_XBOX
	...
	bool GraphicsDevice_DX12::CreateVideoDecoder(const VideoDesc* desc, VideoDecoder* video_decoder) const
	{
		...
		std::vector<GUID> profiles(video_decode_profile_count.ProfileCount);
	...
	void GraphicsDevice_DX12::SparseUpdate(QUEUE_TYPE queue, const SparseUpdateCommand* commands, uint32_t command_count)
	{
		...
		thread_local std::vector<D3D12_TILED_RESOURCE_COORDINATE> tiled_resource_coordinates;
		thread_local std::vector<D3D12_TILE_REGION_SIZE> tiled_region_sizes;
		thread_local std::vector<D3D12_TILE_RANGE_FLAGS> tile_range_flags;
		thread_local std::vector<uint32_t> range_start_offsets;

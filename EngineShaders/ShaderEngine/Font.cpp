...
#include "ThirdParty/liberation_sans.h"
...
namespace vz::font
{
	namespace font_internal
	{
		...
		struct Canvas
		{
			...
			// the following functions are directly from Canvas.h in Core's Common

			// The scaling factor between logical and physical coordinates
			inline float GetDPIScaling() const { return dpi / 96.f; }
		...
		struct FontStyle
		{
			...
			void Create(const std::string& newName, const uint8_t* data, size_t size)
			{
				...
				{
					backlog::post("Failed to load font: " + name + " (file was unrecognized, it must be a .ttf file)");
				}
			...
			{
				if (helper::FileRead(newName, fontBuffer))
				{
					Create(newName, fontBuffer.data(), fontBuffer.size());
				}
				else
				{
					backlog::post("Failed to load font: " + name + " (file could not be opened)");
				}
			}
		};
		static std::vector<std::unique_ptr<FontStyle>> fontStyles;
		...
		static std::unordered_map<int32_t, rectpacker::Rect> rect_lookup;
		struct Bitmap
		{
			int width;
			int height;
	...
	void Initialize()
	{
		...
		static eventhandler::Handle handle1 = eventhandler::Subscribe(eventhandler::EVENT_RELOAD_SHADERS, [](uint64_t userdata) { LoadShaders(); });
		LoadShaders();

		vzlog("font Initialized (%d ms)", (int)std::round(timer.elapsed()));
		isInitialized = true;
	}
	...
	void UpdateAtlas(float upscaling)
	{
		...
		if (!pendingGlyphs.empty())
		{
			...
			static thread_local rectpacker::State packer;
			packer.clear();
			...
			if (packer.pack(4096))
			{
				...
				GetDevice()->SetName(&texture, "font::texture");
			}
			else
			{
	...
	Cursor Draw_internal(const T* text, size_t text_length, const Params& params, CommandList cmd)
	{
		...
		if (status.quadCount > 0)
		{
			...
			if (params.h_align == Alignment::FONTALIGN_CENTER)
				offset.x -= status.cursor.size.x / 2;
			else if (params.h_align == Alignment::FONTALIGN_RIGHT)
				offset.x -= status.cursor.size.x;
			if (params.v_align == Alignment::FONTALIGN_CENTER)
				offset.y -= status.cursor.size.y / 2 * vertical_flip;
			else if (params.v_align == Alignment::FONTALIGN_BOTTOM)
			...
			{
				// Asserts will check that a proper canvas was set for this cmd with image::SetCanvas()

			...
			if (params.isCornerRoundingEnabled())
			{
				...
				for (int i = 0; i < arraysize(params.corners_rounding); ++i)
				{
					...
					for (int j = 0; j < rounding.segments; ++j)
					{
						...
						XMVECTOR bezier = math::GetQuadraticBezierPos(start, B, end, t);
		...
		for (int i = 0; i < STENCILREFMODE_COUNT; ++i)
		{
			for (int d = 0; d < DEPTH_TEST_MODE_COUNT; ++d)
			{
				...
				case STENCILREFMODE_ENGINE:
					dsd.stencil_read_mask = SCU8(STENCILREF_MASK::STENCILREF_MASK_ENGINE);
					break;
				case STENCILREFMODE_USER:
					dsd.stencil_read_mask = SCU8(STENCILREF_MASK::STENCILREF_MASK_USER);
					break;
				default:
					dsd.stencil_read_mask = SCU8(STENCILREF_MASK::STENCILREF_MASK_ALL);
					break;
				}

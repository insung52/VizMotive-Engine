...
namespace vz::shader
{
	...
	void LoadShaders()
	{
		...
		if (device->CheckCapability(GraphicsDeviceCapability::RAYTRACING))
		{
			jobsystem::Execute(CTX_raytracing, [](jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_DDGI_RAYTRACE], "ddgi_raytraceCS_rtapi.cso", ShaderModel::SM_6_5); });
		}
		else
		{
			jobsystem::Execute(CTX_raytracing, [](jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_DDGI_RAYTRACE], "ddgi_raytraceCS.cso"); });
		}
		...
		// SurfelGI compute shaders
		if (device->CheckCapability(GraphicsDeviceCapability::RAYTRACING))
		{
			jobsystem::Execute(CTX_raytracing, [](jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SURFEL_RAYTRACE], "surfel_raytraceCS_rtapi.cso", ShaderModel::SM_6_5); });
		}
		else
		{
			jobsystem::Execute(CTX_raytracing, [](jobsystem::JobArgs args) { LoadShader(ShaderStage::CS, shaders[CSTYPE_SURFEL_RAYTRACE], "surfel_raytraceCS.cso"); });
		}

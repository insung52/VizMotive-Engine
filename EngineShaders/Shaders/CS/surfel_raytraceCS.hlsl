		...
		if (light.IsStaticLight())
		{
			...
			case ENTITY_TYPE_DIRECTIONALLIGHT:
			{
				...
				if (NdotL > 0)
				{
					...
					{
						lightColor *= GetAtmosphericLightTransmittance(GetEnvironment().atmosphere, surface.P, (half3)L, texture_transmittancelut);
					}
			...
			case ENTITY_TYPE_POINTLIGHT:
			{
				...
				if (dist2 < range2)
				{
					...
					if (NdotL > 0)
					{
						...
						lighting.direct.diffuse = (half3)(lightColor * (float)attenuation_pointlight((half)dist2, (half)range, (half)range2));
					}
				}
			}
			break;
				...
				if (dist2 < range2)
				{
					...
					if (NdotL > 0)
					{
						...
						if (spot_factor > spot_cutoff)
						{
							...
							lighting.direct.diffuse = (half3)(lightColor * (float)attenuation_spotlight((half)dist2, (half)range, (half)range2, (half)spot_factor, light.GetAngleScale(), light.GetAngleOffset()));
						}
					}
				}
			}
				...
				case ENTITY_TYPE_POINTLIGHT:
				{
					...
					if (dist2 < range2)
					{
						...
						if (NdotL > 0)
						{
							...
							lighting.direct.diffuse = (half3)lightColor;
							lighting.direct.diffuse *= (half)attenuation_pointlight((half)dist2, (half)range, (half)range2);
						}
					}
				}
				break;
					...
					if (dist2 < range2)
					{
						...
						if (NdotL > 0)
						{
							...
							if (spot_factor > spot_cutoff)
							{
								...
								lighting.direct.diffuse = (half3)lightColor;
								lighting.direct.diffuse *= (half)attenuation_spotlight((half)dist2, (half)range, (half)range2, (half)spot_factor, light.GetAngleScale(), light.GetAngleOffset());
							}
						}
					}
				}
				...
				for (uint i = 0; i < cell.count; ++i)
				{
					...
					if (dist2 < sqr(surfel.GetRadius()))
					{
						...
						if (dotN > 0)
						{
							...
							surfel_gi += float4(SH::CalculateIrradiance(surfel.radiance.Unpack(), (half3)surface.N), 1) * contribution;
						}
					}
				}
				if (surfel_gi.a > 0)

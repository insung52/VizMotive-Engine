...
#include "../CommonHF/volumetricCloudsHF.hlsli"
...
void main(PSInput input)
{
	...
	surface.P = P;
	surface.N = N;
	surface.create(material, (half4)baseColor, 0, 0);
	...
	if (direction_weights.x > 0)
	{
		...
		float2 normal_direction = encode_oct(half3(N * direction_weights.x)) * 0.5 + 0.5;
	...
	if (direction_weights.y > 0)
	{
		...
		float2 normal_direction = encode_oct(half3(N * direction_weights.y)) * 0.5 + 0.5;
	...
	if (direction_weights.z > 0)
	{
		...
		float2 normal_direction = encode_oct(half3(N * direction_weights.z)) * 0.5 + 0.5;

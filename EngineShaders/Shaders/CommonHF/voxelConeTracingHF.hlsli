#ifndef VOXEL_CONERACING_HF
#define VOXEL_CONERACING_HF
...
inline float4 ConeTrace(in Texture3D<half4> voxels, in float3 P, in float3 N, in float3 coneDirection, in float coneAperture, in float stepSize, bool use_sdf = false, uint precomputed_direction = 0)
{
	...
	// Always use clipmap0 voxelSize for the offset — consistent with Wicked Engine.
	// Per-P clipmap adjustment caused level 0 surfaces to use a smaller offset → more self-occlusion
	// → darker result → inverted brightness compared to higher clipmaps + dark outline at boundary.
...
inline float4 ConeTraceSpecular(in Texture3D<half4> voxels, in float3 P, in float3 N, in float3 V, in float roughness, in uint2 pixel)
{
	...
	P += coneDirection * (dither((min16uint2)pixel + GetTemporalAASampleRotation()) - 0.5) * GetFrame().vxgi.stepsize;

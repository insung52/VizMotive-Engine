...
struct Surface
{
	...
	inline void update()
	{
        ...
        NdotV = (half)saturate(dot(N, V) + 1e-5);
		...
		B = (half3)cross(T.xyz, N) * T.w; // Compute bitangent again after normal mapping
	...
	bool preload_internal(PrimitiveID prim)
	{
		...
		Buffer<float4> buf = bindless_buffers_float4[NonUniformResourceIndex(descriptor_index(geometry.vb_pos_w))];
		data0 = buf[i0];
		data1 = buf[i1];
		data2 = buf[i2];
	...
	void load_internal(uint flatTileIndex = 0)	// note that preload_internal is called prior to this
	{
		...
		{
			Buffer<half> buf = bindless_buffers_half[NonUniformResourceIndex(descriptor_index(instResLookup.vb_ao))];
		...
		{
			Buffer<half> buf = bindless_buffers_half[NonUniformResourceIndex(descriptor_index(instResLookup.vb_wetmap))];
